...
for file in os.listdir(path):
    savepath = "lines/" + file[:-4] + "/"
    delpath = "lines/" + file[:-4] + "/not_accepted/"
    ...
    for i, ctr in enumerate(sorted_ctrs):
        ...
        if roi.shape[0] < avg_line_height / 2:
            cv2.imwrite(delpath + 'roi(%d).png' % i, roi)
            cv2.rectangle(deleted, (x, y), (x + w, y + h), (90, 0, 255), 2)
        else:
            cv2.imwrite(savepath + 'roi(%d).png' % i, roi)
