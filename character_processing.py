...
class CharacterProcessing:
    def __init__(self, path_to_image, resize_mode="smallest", model_mode="recognition") -> None:
    ...
    def _load_shape(self):
        ...
        if self.model_mode == "recognition":
            path_to_shape = os.path.join(path_to_letter_folders,"Alef","recognition_shape.txt")
        elif self.model_mode == "style":
            curr_char = os.path.split(self.path_to_image)[1].split("_")[0]
            path_to_shape = os.path.join(path_to_letter_folders,curr_char,"style_shape.txt")
...
for line_folder in os.listdir("data\\TESTING_some_image_name\\"):
    ...
    if line_folder.split('_')[0] == "line":
        for image_in_line_folder in os.listdir(os.path.join("data\\TESTING_some_image_name\\", line_folder)):
            if image_in_line_folder.split("_")[0] == "char":
                path_to_image = os.path.join("data\\TESTING_some_image_name\\", line_folder, image_in_line_folder)
                character_processing = CharacterProcessing(path_to_image, resize_mode=RESIZE_MODE, model_mode="recognition")
    ...
    if line_folder.split('_')[0] == "recognized":
        for image_in_line_folder in os.listdir(os.path.join("data\\TESTING_some_image_name\\", line_folder)):
            if image_in_line_folder.split("_")[1].split("_")[0] == "char":
                path_to_image = os.path.join("data\\TESTING_some_image_name\\", line_folder, image_in_line_folder)
                character_processing = CharacterProcessing(path_to_image, resize_mode=RESIZE_MODE, model_mode="style")
                character_processing.resize_image()