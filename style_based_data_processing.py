...
class StyleBasedTrainDataProcessing:
    ...
    def normalize_data(self, normalization_type="padding", save_mode=False):
        if normalization_type == "padding":
            self._normalize_data_padding(save_mode=save_mode)
        elif normalization_type == "average":
            self._normalize_data_avg_size(save_mode=save_mode)
        elif normalization_type == "smallest":
            self._normalize_data_smallest_size(save_mode=save_mode)

    def _normalize_data_avg_size(self, save_mode=False):
        new_shape = [0, 0]
        count_samples = 0
        for style_class, dict_of_letters in self.dict_of_style_based.items():
            for letter_key, value_list in dict_of_letters.items():
    ...
    def _normalize_data_smallest_size(self, save_mode=False):
        new_shape = [1000, 1000]
        for style_class, dict_of_letters in self.dict_of_style_based.items():
            for letter_key, value_list in dict_of_letters.items():
                for sample in value_list:
    ...
    def _normalize_data_padding(self, save_mode=False):
        biggest_shape = [0, 0]
        for style_class, dict_of_letters in self.dict_of_style_based.items():
            for letter_key, value_list in dict_of_letters.items():
                for sample in value_list:
        ...
        for style_class, dict_of_letters in self.dict_of_style_based.items():
            for letter_key, value_list in dict_of_letters.items():
                ...
                for idx, sample in enumerate(value_list):
                    ...
                    left_pad, right_pad, top_pad, bottom_pad = self._get_paddings(
                        height_diff, width_diff
                    )

                    padding_adder = torch.nn.ZeroPad2d(
    ...
    def _get_paddings(self, height_diff, width_diff):
